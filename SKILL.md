...
This profile is what keeps your rewrites sounding like the author instead of like a generic editor. Update it continuously in Phase 2. While you're at it, skim [references/register-calibration.md](references/register-calibration.md) once — corpus-measured diction, tense, and connector calibration to draw on when composing options; it is calibration, not law, and the author's field norms win.

The same whole-paper read is your one chance to see the structure before you start editing inside it, so map it now. As you go, record in `revision-log.md` under a **Structure notes** heading: each paragraph's topic sentence (or a note that it lacks one), any transition between paragraphs that drops the reader, and a running **term ledger** of key terms, symbols, and acronyms — each one's canonical form, where it's introduced, whether it's used anywhere earlier, and any synonym drift (the same object surfacing as "framework", "pipeline", and "approach" is a reader-stall, not variety). This ledger is what lets you catch use-before-define and terminology-drift defects in Phase 2, which you could never see one sentence at a time.
...
**2. Judge it.** Decide which stop-slop patterns, if any, actually apply, and whether the meaning has any fragile parts you must preserve — hedges, bounds, quantifiers, units, numbers, defined terms, polarity, and causal direction ("may be associated with" flattened into "caused" is a rejected rewrite, not a style win). Then run three quick editorial lenses:

- **Job.** A sentence should do one job — background, gap, method, result, comparison, implication, or limitation. Two jobs crammed into one sentence is a reason to split or subordinate, and a natural Bold.
- **Hedge and verb.** Hedges belong on interpretation, not observation: the observation sentence reads assertive with its number, and the "may"/"suggests" moves to the meaning sentence; verbs should sit on the rung the evidence supports (show/demonstrate ≫ suggest/indicate ≫ may reflect) — see [references/register-calibration.md](references/register-calibration.md). Repositioning a hedge or moving a verb up the ladder touches claim strength, so offer it only as an explicitly flagged option, never folded silently into a rewrite.
- **Overclaim.** Flag prove, conclusively, unprecedented, best, superior, unqualified first — and a claim missing its boundary, or correlation dressed as mechanism. Propose a bounded alternative ("to our knowledge", "in this system") and let the author decide, exactly like a use-before-define flag.

Read it once more as the reader, not the author: is there jargon with a plainer equivalent, an acronym not yet expanded, a term used here that your term ledger says isn't defined until later, or a term drifting from its ledgered form? Those are reader-stalls worth fixing even when the sentence is otherwise clean. If the style profile marks the author as writing in a second language (or the patterns keep appearing), add the interference lens: missing or misused articles, comma-spliced clause chains, topic-comment word order, a topic noun repeated where English wants a pronoun, vague attributions ("many studies have shown"), and hedging calibrated lower than the evidence — and when restoring a logical link, use the smallest connective that does the job, never a mechanical one. If the sentence is both clean and clear to an outside reader and in the author's voice, don't manufacture problems to look busy: in stop-at-every-sentence mode, say so plainly and offer to keep it — a fast "this one's clean, keep it?" is a good outcome; in flag-only mode, log it as `kept` and move on, reporting skipped ranges at the next stop. When the sentence opens or closes a paragraph, also weigh its structural job: does the opener announce the paragraph's claim, does the closer hand off to the next paragraph? Let that shape the rewrites.

**3. Offer three versions as a spectrum.** When a sentence does need work, generate exactly three rewrites at increasing edit distance, each obeying the style-profile and the term ledger (never introduce a synonym for a ledgered term to vary the prose — elegant variation is its own tell), and each changing nothing about the meaning:
...
**Paragraph checkpoints.** When you finish the last sentence of a paragraph, pause for a short paragraph-level look — the view the sentence loop can't give you. Four questions: does the paragraph lead with a clear topic sentence and support it, or is the claim buried? Does it flow from the previous paragraph and into the next, or is a transition missing or merely mechanical? Does any term land before it's defined — or drift from its ledgered form — per your term ledger? And is the closing sentence sagging — paragraph-final sentences are often the longest and weakest? If something's off, offer a paragraph-level fix the same way you offer sentence edits — propose a concrete change (promote or rewrite the topic sentence, add a real transition, move a definition earlier), let the author choose with `AskUserQuestion`, and log it against the relevant sentence ids. Keep these checkpoints lighter and rarer than the sentence loop; most paragraphs will pass.
